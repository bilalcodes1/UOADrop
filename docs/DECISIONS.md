...
| P7 | **الإشعارات الحالية** | محلياً توجد إشعارات نظام داخل Electron؛ Online يدعم Email/Telegram وإعلان جماعي للأونلاين فقط |
...
- `apps/web` هو تطبيق Next.js لمسار Online وواجهات إشعارات Email/Telegram.
...
- زر `جاهز` ينقل الطلب إلى تبويب الجاهز ويمسح الفلاتر المخفية.
- زر `تم التسليم` ينقل الطلب إلى الأرشيف.
- حذف طلب Online يحدّث Mirror في Supabase إلى `canceled` قبل الحذف المحلي.
...
- عند تفعيل مسار Online المشفر، تُشفّر ملفات الطالب في المتصفح قبل Supabase Storage باستخدام AES-256-GCM + RSA-OAEP-SHA256.
- الإعلان الجماعي يستهدف Online فقط عبر `student_email` و`telegram_chat_id` من Supabase، ولا يدخل طلبات Offline/Local.

---

## 9. ما يزال خارج نطاق النسخة الحالية

هذه البنود **مؤجلة أو غير مكتملة بالكامل**:

- توقيع Windows/macOS الرسمي.
- إدارة تنظيف التخزين بالكامل من داخل Supabase بدون الاعتماد على تشغيل الديسكتوب.
- مزامنة عكسية كاملة لكل تفاصيل الديسكتوب إلى السحابة خارج mirror الحالي.
