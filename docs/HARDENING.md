...
> **ملاحظة مرجعية:** هذا الملف تاريخي/مرجعي ويجمع نقاشات hardening عبر عدة جولات. بعض البنود مطبّق فعلاً في التطبيق الحالي، وبعضها يخص مراحل online أو تصاميم لاحقة أو توصيات تصميمية سابقة. المرجع الأدق للحالة الحالية هو `ARCHITECTURE.md` و `DECISIONS.md` و `SETUP.md` و `README.md`، خصوصاً في مسائل تشغيل LAN الحالية وخدمة الأصول المحلية والواجهة الطالبية.
...
- **المشكلة**: `evil.exe` → rename → `report.pdf` → يمر الفحص. صاحب المكتب يفتحه.
...
- **المشكلة**: `powerSaveBlocker` يمنع النوم لما الشاشة مفتوحة فقط. صاحب المكتب يغلق الغطاء → النظام ميت.
- **الحل**:
...
- **المشكلة**: لو صاحب المكتب أخذ اللابتوب لشبكة عامة، أي شخص على نفس الـ Wi-Fi يقدر يطلب.
- **الحل**: bind على `192.168.0.100` فقط.
- **ملاحظة على التنفيذ الحالي**: النسخة الحالية تربط الخادم على `0.0.0.0` عمداً لأن مسار التشغيل الرسمي هو داخل شبكة المكتب المحلية، مع الاعتماد على إعداد الشبكة المحلية والجدار الناري والتعليمات التشغيلية في `SETUP.md`.
- **موثّق في**: `ARCHITECTURE.md §8`.
...
### C17. لا session timeout لـ Dashboard صاحب المكتب
- **المشكلة**: صاحب المكتب يترك اللابتوب مفتوحاً → متطفل يعدّل أسعار، يحذف طلبات، يقرأ PINs.
...
- **المشكلة**: الطالب داخل المكتب فيزيائياً عند صاحب المكتب → PIN لا يضيف أمان. ولو أقفلت صفحة التأكيد = مقفولة.
...
- **المشكلة**: PIN hash فقط → لا يمكن استرجاعه. الطالب نسيه = مقفول.
...
- **المشكلة**: الطالب لا يقدر يلغي قبل printing.
...
- **الحل**: 3 طبقات — mDNS `uoadrop.local` + تعليمات نصية بجانب QR + رقم صاحب المكتب.
...
- **ملاحظة على التنفيذ الحالي**: الواجهة المحلية الحالية للطالب والدشبورد لا تعتمد على Google Fonts أو CDN خارجي لكي تعمل داخل الشبكة المحلية بدون إنترنت.
